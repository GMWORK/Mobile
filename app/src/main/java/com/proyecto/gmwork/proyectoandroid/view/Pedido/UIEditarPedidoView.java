 ...
 */
public class UIEditarPedidoView extends ActionBarActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
...
//        adapter = new AdapterListPedidoProductos(this, R.layout.lista_pedidoproducto_adapter);
    ...
    private void setResources() {
        ...
        btn_finish = (Button) findViewById(R.id.aep_btn_finish);
        per = new PersistencyController(this);
        tv_cantidad = (TextView) findViewById(R.id.aep_tv_cantidad);
        spn_productos = (Spinner) findViewById(R.id.aep_sp_productos);
        number = (SeekBar) findViewById(R.id.aep_sb_cantidad);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.aep_btn_finish:
                finish();
                break;
            case R.id.aep_btn_crPedido:
                per.editarPedido(idPedido, tv_Cliente.getText().toString(), DateTime.now().plusMonths(1).toString(), adapter);
                finish();
                break;
